...
- ETAPA 5 autorizada: seed idempotente dos quatro planos padrão em `scripts/seed_plans.py`, por `code` único. Inserir ausentes e atualizar exclusivamente `price` quando divergente dos preços oficiais: MONTHLY 79.90, SEMIANNUAL 399.90, ANNUAL 699.90, LIFETIME 1499.90 (Decimal/BRL). Preservar IDs, demais campos e relacionamentos existentes.
